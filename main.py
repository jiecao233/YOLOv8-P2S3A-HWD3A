...
def main():
    ...
    # Ball Tracker
    ball_tracker = Tracker('models/ball_best.pt')
    # Player Tracker
    player_tracker = Tracker('models/player_best.pt')
    ...
    ball_tracker.add_position_to_tracks(ball_tracks)
    ...
    camera_movement_per_frame = camera_movement_estimator.get_camera_movement(video_frames,
                                                                              read_from_stub=True,
                                                                              stub_path='stubs/camera_movement_stub.pkl')
    camera_movement_estimator.add_adjust_positions_to_tracks(ball_tracks, camera_movement_per_frame)
    ...
    view_transformer.add_transformed_position_to_tracks(ball_tracks)
    view_transformer.add_transformed_position_to_tracks(player_tracks)

    # Interpolate Ball Positions
    #tracks["ball"] = tracker.interpolate_ball_positions(tracks["ball"])
    ball_tracks["ball"] = ball_tracker.interpolate_ball_positions(ball_tracks["ball"])

    # Speed and distance estimator
    speed_and_distance_estimator = SpeedAndDistance_Estimator()
    #speed_and_distance_estimator.add_speed_and_distance_to_tracks(tracks)
    speed_and_distance_estimator.add_speed_and_distance_to_tracks(ball_tracks)
    ...
    team_ball_control= []
    for frame_num, player_track in enumerate(player_tracks['players']):
        ball_bbox = ball_tracks['ball'][frame_num][1]['bbox']
    ...
    # Draw output 
    ## Draw object Tracks
    output_video_frames = ball_tracker.draw_annotations(video_frames, ball_tracks,team_ball_control)
    ...
    speed_and_distance_estimator.draw_speed_and_distance(output_video_frames, ball_tracks)
